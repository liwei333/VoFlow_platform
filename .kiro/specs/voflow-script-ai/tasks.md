...
- [ ] 4. 接入 ASR Worker handler
  - 从对象存储下载音视频
  - 调用 faster-whisper 或 mock provider
  - 输出文本和 segments
  - _Requirements: US-2_
...
- [ ] 7. 实现本地 LLM 配置读取
  - 从环境变量读取 `LLM_PROVIDER`
  - 从环境变量读取 `LLM_BASE_URL`、`LLM_MODEL`、`LLM_TIMEOUT_MS`、`LLM_MAX_TOKENS`
  - 不要求配置线上模型密钥
  - _Requirements: US-5_

- [ ] 8. 实现本地 LLM 健康检查
  - 实现 `GET /api/llm/health`
  - 检查本地模型服务连接和模型名
  - 不可用时返回 `LOCAL_LLM_UNAVAILABLE`
  - _Requirements: US-5_

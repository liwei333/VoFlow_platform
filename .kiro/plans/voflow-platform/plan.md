...
| 13 | 系统生成各平台标题、标签、描述、话题 | `voflow-publish-assistant` | partial | Task 0 已完成后续开发规范检查；Task 1 已新增发布相关数据表和 Prisma enum；Task 2 已新增平台规则模块；Task 3 已新增本地 OpenAI-compatible 发布草稿 LLM provider、`generatePublishDrafts()` service 和 `POST /api/video-jobs/[jobId]/publish-drafts/generate`；Task 4 已新增 `GET /api/video-jobs/[jobId]/publish-drafts` 和 `PUT /api/publish-drafts/[draftId]`，可查询/编辑平台独立草稿并校验 title/tags/cover，同步 serializer/service 输出口径；Task 5 已新增渠道账号状态展示 API 和 token 加密存储 helper；Task 6 已新增 OAuth/mock 授权占位入口；Task 7 已新增 `POST /api/video-jobs/[jobId]/publish/validate` 和 `publish_drafts.validationJson` 写回；Task 8 已新增 publish `ChannelAdapter` 接口和 mock adapter；Task 9 已新增 `POST /api/video-jobs/[jobId]/publish`，可对检查通过且 connected 的平台创建发布记录，并对 expired/not_connected 平台写入 skipped；Task 10 已新增 `POST /api/publishes/[publishId]/retry`，只允许 failed 发布记录重试并保存错误历史；Task 11 已将 `/dashboard/publish` 从占位页替换为发布信息编辑 UI，支持平台 tabs、标题/标签/描述/话题/封面字段编辑和来自 `draft.rule` 的限制提示；14 个 publish focused test 文件 54 个测试通过 | 继续 Task 12：实现发布中心 UI |
| 14 | 用户一键发布到已授权平台，未授权或 token 过期平台给出可处理状态 | `voflow-publish-assistant` | partial | Task 0 已完成启动规范检查；Task 1 已建立渠道账号、发布草稿和发布记录的数据基础；Task 2 已建立后续发布参数检查可复用的平台规则模块；Task 3 已建立生成草稿 API；Task 4 已建立发布草稿查询/编辑 API；Task 5 已新增渠道账号状态 serializer/service、AES-256-GCM token 加密 helper、`GET /api/channel-accounts`，可展示 connected/expired/not_connected、账号昵称和过期时间；Task 6 已新增 `GET /api/channel-accounts/[platform]/authorize` 和 `POST /api/channel-accounts/[platform]/mock-authorize`，可返回 mock 授权/重新授权入口并创建或覆盖 encrypted mock channel account；Task 7 已新增发布前检查，校验标题、标签、封面比例、最终视频比例/时长和 channel account 状态，并对失败平台返回 `PUBLISH_VALIDATION_FAILED`；Task 8 已新增 `uploadVideo`、`publish`、`getStatus`、`retry` Adapter 接口和 mock adapter，当前不代表真实平台发布能力；Task 9 已新增一键发布 API，复用 mock adapter 保存 requestId/remoteId，对 token 过期或未授权平台落库 skipped 并保存原因；Task 10 已新增失败重试 API，复用 mock adapter.retry 更新单条 failed 发布记录、保存新 requestId/remoteId，并将原错误写入 `errorJson.history`；Task 11 已新增发布信息编辑 UI；14 个 publish focused test 文件 54 个测试通过 | 继续 Task 12：实现发布中心 UI；尚未实现账号/校验/日志总控 UI 和真实平台发布 |
...
- Active Scope: `voflow-publish-assistant` Task 0-11 已完成，已建立发布数据基础、平台规则配置、发布草稿生成/查询/编辑 API、渠道账号状态展示 API、token 加密存储 helper、OAuth/mock 授权占位接口、发布参数检查 API、mock Channel Adapter、一键发布 API、失败重试 API 和发布信息编辑 UI
- Active Focus: `voflow-publish-assistant` Task 12 实现发布中心 UI
- Status: `ready_for_next_task`

未完成当前游标前，不允许并行启动当前入口之外的新 Spec。`voflow-publish-assistant` 已是当前唯一开发入口。
...
| `voflow-publish-assistant` | partial | 继续 Task 12：实现发布中心 UI |

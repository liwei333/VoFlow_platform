...
| 13 | 系统生成各平台标题、标签、描述、话题 | `voflow-publish-assistant` | partial | Task 0-14 已完成 checkpoint；已新增发布相关数据表、平台规则模块、本地 OpenAI-compatible 发布草稿 provider、`generatePublishDrafts()` service、草稿生成/查询/编辑 API、发布信息编辑 UI、发布中心操作台 UI 和发布辅助 acceptance 测试；15 个 publish focused test 文件 60 个测试、lint、build、diff check 均通过 | 最终 MVP 环境复验本地 LLM 服务；真实平台发布链路仍未接入 |
| 14 | 用户一键发布到已授权平台，未授权或 token 过期平台给出可处理状态 | `voflow-publish-assistant` | partial | Task 0-14 已完成 checkpoint；已新增渠道账号状态 serializer/service、AES-256-GCM token 加密 helper、`GET /api/channel-accounts`、mock OAuth 授权/重新授权入口、发布前检查 API、mock `PublishChannelAdapter`、一键发布 API、失败重试 API、账号/校验/日志总控 UI；15 个 publish focused test 文件 60 个测试、lint、build、diff check 均通过 | 真实开放平台 OAuth/Adapter/发布状态同步尚未接入，不能按真实平台发布能力标记为 done |
...
- Active Scope: `voflow-publish-assistant` Task 0-14 已完成，已建立发布数据基础、平台规则配置、发布草稿生成/查询/编辑 API、渠道账号状态展示 API、token 加密存储 helper、OAuth/mock 授权占位接口、发布参数检查 API、mock Channel Adapter、一键发布 API、失败重试 API、发布信息编辑 UI、发布中心操作台 UI、发布辅助 acceptance 测试和 checkpoint 验收记录
- Active Focus: `voflow-publish-assistant` checkpoint 已完成；后续为真实平台 OAuth/Adapter 接入、发布状态同步增强和最终 MVP 环境复验
- Status: `checkpoint_complete_mock_boundary`
...
| `voflow-publish-assistant` | partial | checkpoint 已完成；最终 MVP 环境复验或真实平台 Adapter 接入 |
